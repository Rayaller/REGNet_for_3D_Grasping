...
#define CHECK_GE(x, y) TORCH_CHECK((x) >= (y), #x " is not greater than or equal to " #y)
...
std::vector<at::Tensor> PointSearch(
  ...
  auto index = at::zeros(
      {batch_size, num_query, num_neighbours},
      query_xyz.options().dtype(at::kLong));
  ...
  AT_DISPATCH_FLOATING_TYPES(query_xyz.scalar_type(), "PointSearch", ([&] {
    PointSearchKernel<scalar_t, int64_t>
      <<<batch_size, block>>>(
        query_xyz_trans.data_ptr<scalar_t>(),
        key_xyz_trans.data_ptr<scalar_t>(),
