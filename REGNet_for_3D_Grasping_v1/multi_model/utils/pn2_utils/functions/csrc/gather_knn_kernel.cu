...
at::Tensor GatherKNNForward(
  ...
  const auto num_idx = index.size(1);
...
//  CHECK_EQ(index.size(1), num_inst);
  ...
  auto input_expand = input.unsqueeze(2).expand({batch_size, channels, num_idx, num_inst});  // (B, C, N, N)
  auto index_expand = index.unsqueeze(1).expand({batch_size, channels, num_idx, k});  // (B, C, N, K)
...
//  CHECK_EQ(index.size(1), num_inst);
  ...
  // Allocate grasp_proposal space for output
  ...
  AT_DISPATCH_FLOATING_TYPES(grad_output.scalar_type(), "GatherKNNBackward", ([&] {
    ...
    GatherKNNBackwardKernel<scalar_t, uint64_t><<<grid, block>>>(
        gradInputInfo,
        gradOutputInfo,
        IndexInfo,
